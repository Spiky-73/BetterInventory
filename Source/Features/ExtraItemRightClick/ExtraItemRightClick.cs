...
public sealed class ExtraItemRightClickPlayer : ModPlayer {
    ...
    public override bool IsLoadingEnabled(Mod mod) => Compatibility.LoadDisabledFeatures || FeaturesConfig.ExtraItemRightClick;
    public override void Load() {
        On_ItemSlot.PickupItemIntoMouse += HookNoPickupMouse;
    }
    public override bool PreItemCheck() {
        if (!FeaturesConfig.ExtraItemRightClick || Main.myPlayer != Player.whoAmI) return true;
    ...
    private void HookNoPickupMouse(On_ItemSlot.orig_PickupItemIntoMouse orig, Item[] inv, int context, int slot, Player player) {
        if (!FeaturesConfig.ExtraItemRightClick || !s_noMousePickup) orig(inv, context, slot, player);
    }
