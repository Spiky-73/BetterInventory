...
public sealed class MinimumDisplayedInfo : ILoadable {
    ...
    private static void ILSearchAddEntries(ILContext il) {
        ...
        cursor.EmitDelegate((BestiaryUICollectionInfo info) => {
            if (BetterBestiaryConfig.MinimalDisplayedInfo && info.UnlockState > BestiaryEntryUnlockState.NotKnownAtAll_0) info.UnlockState = GetDisplayedUnlockLevel(info.UnlockState);
            return info;
        });
        // ...
    }
