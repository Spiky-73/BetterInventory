...
public sealed class StackTrash : ILoadable {
    ...
    public bool IsLoadingEnabled(Mod mod) => Compatibility.LoadDisabledFeatures || BetterTrashConfig.StackTrash;
    public void Load(Mod mod) {
        IL_ItemSlot.SellOrTrash += static il => il.TryEdit(ILStackTrash, ref UnloadedBetterTrashConfig.Instance.stackTrash);
    ...
    private static void ILStackTrash(ILContext il) {
        ...
        cursor.EmitDelegate((Item trash) => {
            if (!BetterTrashConfig.StackTrash || trash.type != Main.LocalPlayer.trashItem.type) return trash;
    ...
    private static int HookStackSold(On_Chest.orig_AddItemToShop orig, Chest self, Item newItem) {
        ...
        if (!BetterTrashConfig.StackTrash || bought >= newItem.stack) return orig(self, newItem);
