...
public sealed class UnknownNPCs : ILoadable {
    ...
    public void Load(Mod mod) {
        ...
        IL_UIBestiaryEntryIcon.Update += il => il.TryEdit(ILIconUpdateFakeUnlock, ref UnknownEntitiesConfig.Instance.bestiary);
        IL_UIBestiaryEntryIcon.DrawSelf += il => il.TryEdit(ILIconDrawFakeUnlock, ref UnknownEntitiesConfig.Instance.bestiary);
        IL_UIBestiaryEntryInfoPage.AddInfoToList += il => il.TryEdit(IlEntryPageFakeUnlock, ref UnknownEntitiesConfig.Instance.bestiary);
        IL_UIBestiaryFilteringOptionsGrid.UpdateAvailability += il => il.TryEdit(ILFakeUnlockFilters, ref UnknownEntitiesConfig.Instance.bestiary);
    }
    ...
    private static void ILIconUpdateFakeUnlock(ILContext il) {
        ...
        cursor.EmitDelegate((BestiaryUICollectionInfo info) => {
            if (UnknownEntitiesConfig.Bestiary && UnknownEntitiesConfig.Instance.unknownDisplay == UnknownDisplay.Known && info.UnlockState < BestiaryEntryUnlockState.CanShowPortraitOnly_1) info.UnlockState = BestiaryEntryUnlockState.CanShowPortraitOnly_1;
            return info;
    ...
    private static void ILIconDrawFakeUnlock(ILContext il) {
        ...
        // ++ <changeVisibleState>
        cursor.EmitDelegate((bool unlocked) => unlocked || (UnknownEntitiesConfig.Bestiary && UnknownEntitiesConfig.Instance.unknownDisplay == UnknownDisplay.Known));
    }
    private static void HookDarkenEntryButton(On_UIBestiaryEntryButton.orig_ctor orig, UIBestiaryEntryButton self, BestiaryEntry entry, bool isAPrettyPortrait) {
        ...
        if (!UnknownEntitiesConfig.Bestiary || self.Entry.Icon.GetUnlockState(self.Entry.UIInfoProvider.GetEntryUICollectionInfo())) return;
    ...
    private static void HookDarkenEntryPage(On_UIBestiaryEntryInfoPage.orig_AddInfoToList orig, UIBestiaryEntryInfoPage self, BestiaryEntry entry, ExtraBestiaryInfoPageInformation extraInfo) {
        ...
        if (!UnknownEntitiesConfig.Bestiary) return;
    ...
    private static void ILFakeUnlockFilters(ILContext il) {
        ...
        cursor.EmitDelegate((bool on, IBestiaryEntryFilter filter, List<BestiaryEntry> entries) => {
            s_ilOn = on;
            if (!UnknownEntitiesConfig.Bestiary || on || filter.ForcedDisplay.HasValue) return false;
            if (UnknownEntitiesConfig.Instance.unknownDisplay == UnknownDisplay.Known) {
                s_ilOn = true;
                return false;
            }
            if (UnknownEntitiesConfig.Instance.unknownDisplay == UnknownDisplay.Hidden) {
                s_ilSkipped++;
                return true;
            }
            return false;
    ...
    private static void HookBestiaryFilterRemoveHiddenEntries(On_UIBestiaryTest.orig_FilterEntries orig, UIBestiaryTest self) {
        orig(self);
        if (!UnknownEntitiesConfig.Bestiary || UnknownEntitiesConfig.Instance.unknownDisplay != UnknownDisplay.Hidden) return;
