...
public sealed class FavoriteInBanksPlayer : ModPlayer {
    ...
    public static void OnConfigChanged() {
        ItemSlot.canFavoriteAt[ItemSlot.Context.BankItem] = BetterInventoryManagementConfig.FavoriteInBanks;
    }
    ...
    private static void ILKeepFavoriteInBanks(ILContext il) {
        ...
        cursor.EmitDelegate((bool fav, Item[] inv, int context, int slot) => {
            if (BetterInventoryManagementConfig.FavoriteInBanks && context == ItemSlot.Context.BankItem) fav = inv[slot].favorited;
            return fav;
        });
    }
    private static void ILFavoritedBankBackground(ILContext il) {
        ...
        cursor.EmitDelegate((Texture2D texture, Item[] inv, int context, int slot) => {
            if (!BetterInventoryManagementConfig.FavoriteInBanks || context != ItemSlot.Context.BankItem || !inv[slot].favorited) return texture;
    ...
    private static void HookChestAction(Action orig) {
        ...
        if (!sync && BetterInventoryManagementConfig.FavoriteInBanks) ItemHelper.RunWithHiddenItems(items, () => orig(), i => i.favorited);
        else orig();
    }
