...
public sealed class QuickMovePlayer : ModPlayer {
    ...
    public override void ProcessTriggers(TriggersSet triggersSet) {
        if (!BetterInventoryManagementConfig.QuickMove) return;
    ...
    public override bool HoverSlot(Item[] inventory, int context, int slot) {
        if (!BetterInventoryManagementConfig.QuickMove) return false;
...
public static class QuickMove {
    ...
    private static List<QuickMovedItem> _movedItems = [];
    ...
    private static List<QuickMovedItem> Move(InventorySlot source, InventorySlot target) {
        ...
        List<Item> freeItems = [];
        List<QuickMovedItem> movedItems = [new(source, target.Inventory, item.type, item.prefix, item.favorited)];
    ...
    private static void UndoMove(List<QuickMovedItem> movedItems) {
        foreach (QuickMovedItem moved in movedItems) {
...
public readonly record struct QuickMovedItem(InventorySlot From, ModSubInventory To, int Type, int Prefix, bool Favorited);
