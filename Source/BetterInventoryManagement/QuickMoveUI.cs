...
public sealed class QuickMoveUIItem : GlobalItem {
    ...
    private static void HookItemSlotHover(On_ItemSlot.orig_MouseHover_ItemArray_int_int orig, Item[] inv, int context, int slot) {
        ...
        _hovering = true;
        QuickMoveUI.SetupChain(itemSlot);
    }
    ...
    private static void HookItemSlotDraw(On_ItemSlot.orig_Draw_SpriteBatch_ItemArray_int_int_Vector2_Color orig, SpriteBatch spriteBatch, Item[] inv, int context, int slot, Vector2 position, Color lightColor) {
        ...
        QuickMoveChainSlotDisplay display;
        if (QuickMove.InChain()) {
            if (!QuickMoveUI.TryGetPlayerMoveChainSlot(itemSlot, out display)) return;
        } else {
            if (!QuickMoveUI.TryGetMoveChainSlot(itemSlot, out display)) return;
        }

        string? text = QuickMoveUI.GetDisplayedHotkey(display);
    ...
    public override void ModifyTooltips(Item item, List<TooltipLine> tooltips) {
        if (!QuickMoveConfig.ItemTooltip || !QuickMoveUI.InChain()) return;

        tooltips.Add(new(Mod, "QuickMove", QuickMoveUI.GetTooltip(QuickMoveUI.Chain())));
    }

    private static bool DisplayChainHotkeys => _hovering ? QuickMoveUI.InChain() : QuickMove.InChain();
...
public static class QuickMoveUI {
    ...
    public static ReadOnlyDictionary<InventorySlot, QuickMoveChainSlotDisplay> ChainSlots() => _chainSlots.AsReadOnly();
    public static bool TryGetMoveChainSlot(InventorySlot slot, out QuickMoveChainSlotDisplay display) => ChainSlots().TryGetValue(slot, out display);
    public static bool TryGetPlayerMoveChainSlot(InventorySlot slot, out QuickMoveChainSlotDisplay display) {
        display = default;
    ...
    public static string? GetDisplayedHotkey(QuickMoveChainSlotDisplay slot) {
    ...
    private static Dictionary<InventorySlot, QuickMoveChainSlotDisplay> _chainSlots = [];
}

public record struct QuickMoveChainSlotDisplay(int MoveKey, int Presses);