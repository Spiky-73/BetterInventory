...
public class RecipeTooltipItem : GlobalItem {
    ...
    private static void HookClearAvailableRecipes(On_Recipe.orig_ClearAvailableRecipes orig) {
        if (BetterRecipeListConfig.RecipeTooltip) RecipeTooltip.PreClearAvailableRecipes();
        orig();
    }

    private static void HookCollectGuideRecipes(On_Recipe.orig_CollectGuideRecipes orig) {
        if (BetterRecipeListConfig.RecipeTooltip) RecipeTooltip.PreCollectGuideRecipes();
        orig();
    }

    private static void HookItemGroupName(On_ItemTagHandler.ItemSnippet.orig_ctor orig, TextSnippet self, Item item) {
        if (BetterRecipeListConfig.RecipeTooltip) RecipeTooltip.ModifyRecipeGroupName(item);
        orig(self, item);
    }

    public override void ModifyTooltips(Item item, List<TooltipLine> tooltips) {
        if (BetterRecipeListConfig.RecipeTooltip) RecipeTooltip.ModifyTooltips(item, tooltips);
    }
}
