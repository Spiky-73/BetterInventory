...
public sealed class RequiredObjectsDisplay : ILoadable {
    ...
    private static void HookGuideCraftText(On_Main.orig_DrawGuideCraftText orig, int adjY, Color craftingTipColor, out int inventoryX, out int inventoryY) {
        ...
        // Update if needed
        if (Main.numAvailableRecipes == 0 || UnknownRecipesPlayer.IsUnknown(recipe.RecipeIndex)) return;
        ...
        void HandleObjectSlot(Item[] inv, int slot) {
            ...
            if (Configs.VanillaPatchesConfig.MaterialsWrapping && ++number % TilesPerLine == 0) {
                position.X = minX;
                position.Y += delta.Y;
