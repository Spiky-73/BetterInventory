...
public static class Utility {
    ...
    public static Item? LastStack(this Player player, int type, HashSet<Item> avoid) {
        bool Check(Item i) => !avoid.Contains(i) && i.type == type && i.stack > 0;
        ...
        if (Check(player.inventory[InventorySlots.Mouse])) return player.inventory[InventorySlots.Mouse];
        return null;
    }

    public static Item? SmallestStack(this Player player, int type, HashSet<Item> avoid) {
        bool Check(Item i, Item? min) => !avoid.Contains(i) && i.type == type && i.stack > 0 && (min is null || i.stack < min.stack);
        
        Item? min = null;
        for (int i = InventorySlots.Items.End - 1; i >= InventorySlots.Items.Start; i--) if(Check(player.inventory[i], min)) min = player.inventory[i];
        for (int i = InventorySlots.Ammo.End - 1; i >= InventorySlots.Coins.Start; i--) if(Check(player.inventory[i], min)) min = player.inventory[i];
        if(Check(player.inventory[InventorySlots.Mouse], min)) min = player.inventory[InventorySlots.Mouse];
        return min;
    }
